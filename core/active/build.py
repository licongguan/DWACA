...
def RegionSelection(cfg, feature_extractor, classifier, tgt_epoch_loader, mIoUs):

    feature_extractor.eval()
    classifier.eval()

    floating_region_score = FloatingRegionScore(mIoUs, in_channels=cfg.MODEL.NUM_CLASSES, size=2 * cfg.ACTIVE.RADIUS_K + 1).cuda()
